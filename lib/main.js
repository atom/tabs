  ...
  activate () {
    ...
    Object.keys(paneContainers).forEach(location => {
      ...
      this.subscriptions.add(container.observePanes(pane => {
        ...
        pane.onDidDestroy(() => this.tabBarViews.splice(this.tabBarViews.indexOf(tabBarView), 1))
        this.mruListViews.push(mruListView)
        pane.onDidDestroy(() => this.mruListViews.splice(this.mruListViews.indexOf(mruListView), 1))
      }))
    })
  },
