...
export default class MRUItemView {
  initialize (listView, item) {
    ...
    if (typeof item.getIconName === 'function') {
      ...
      let addIconToElement = IconServices.get('element-icons')
      if (addIconToElement) {
        this.disposables = this.disposables || new CompositeDisposable()
        const iconDisposable = addIconToElement(firstLineDiv, itemPath)
        this.disposables.add(iconDisposable)
      } else {
        let typeClasses = IconServices.get('file-icons').iconClassForPath(itemPath, 'tabs-mru-switcher')
        if (typeClasses) {
